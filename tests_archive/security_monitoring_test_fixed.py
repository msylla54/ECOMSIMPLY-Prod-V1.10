    ...
    async def test_password_security_system(self):
        ...
        try:
            ...
            async with self.session.post(f"{BACKEND_URL}/auth/login", json=login_data) as response:
                if response.status == 200:
                    response_data = await response.json()
                    self.test_user_token = response_data.get("token")
                    self.log_test_result(
                    ...
                    if response.status == 200:
                        ...
                        if endpoint == "/health":
                            if "status" in response_data and response_data["status"] == "healthy":
                                uptime = response_data.get("system", {}).get("uptime", 0)
                                services = response_data.get("services", {})
                                
                                self.log_test_result(
                                    f"Health Endpoint {endpoint}",
                                    True,
                                    f"{description} - Status: {response_data.get('status')}, Uptime: {uptime:.1f}s, Services: {services}",
                                    response_data
                                )
                            else:
                                self.log_test_result(
                                    f"Health Endpoint {endpoint}",
                                    False,
                                    f"{description} - Unexpected response structure",
                                    response_data
                                )
                                
                        elif endpoint == "/health/ready":
                            if "status" in response_data and response_data["status"] == "ready":
                                self.log_test_result(
                                ...
                                self.log_test_result(
                                    f"Readiness Check {endpoint}",
                                    False,
                                    f"{description} - Not ready: {response_data}",
                                    response_data
                                )
                                
                        elif endpoint == "/health/live":
                            if "status" in response_data and response_data["status"] == "alive":
                                self.log_test_result(
                                ...
                                self.log_test_result(
                                    f"Liveness Check {endpoint}",
                                    False,
                                    f"{description} - Not alive: {response_data}",
                                    response_data
                                )
                            ...
                            self.log_test_result(
                                ...
                                has_metrics,
                                f"{description} - Contains system metrics: {list(response_data.keys())}",
                ...
                if response.status == 200:
                    ...
                    if db_status == "healthy":
                        ...
                        self.log_test_result(
                            ...
                            True,
                            f"Database connection status: {db_status}",
                            {"status": db_status}
                        )
                    else:
                        self.log_test_result(
                            "Production Database Configuration",
                            False,
                            f"Database status unclear: {db_status}",
                            {"database_status": db_status}
                        )
                        ...
                        self.log_test_result(
                            "Database Connection Stability",
                            False,
                            f"Database connection not healthy: {db_status}",
                            {"status": db_status}
                        )
                else:
                    error_text = await response.text()
                ...
                async with self.session.get(f"{BACKEND_URL}/admin/activity-logs?admin_key=ECOMSIMPLY_ADMIN_2024", headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.log_test_result(
                            ...
                            logs_count >= 0,  # Accept 0 or more logs
                            f"Activity logs accessible: {logs_count} entries",
